...
@section('content')
	...
	<div class="container">
		<ul class="list-group shadow my-2">
			<li class="list-group-item">
				...
				<div class="col-md-12 col-lg-12 col-sm-12 text-center">
					<h1><u> Waysale Voucher: <span class="text-primary">{{ $waysale_details->b_short }}-{{ $waysale_details->voucher_no }}</span> </u></h1>
				</div>

				<div class="col-md-12 col-lg-12 col-sm-12">
					<a href=" {{route('way_sale_detail',$waysale_details->wayout_id)}} " class="btn btn-outline-warning"> Back </a>
				</div>
		...
		<form method="post" action="{{route('waycreditsales.update',$waysale_details->id)}}" class="form-group">
		@csrf
		@method('PUT')
		<ul class="list-group shadow my-2">
			<li class="list-group-item">
			...
			<div class="row">
				<div class="col-md-6 col-lg-6">
					...
					<select name="customer_name" id="customer_name" class="form-control @error ('customer_name') @enderror">
						...
						@foreach($customers as $customer)
							...
							<option value="{{$customer->id}}" @if($waysale_details->customer_id == $customer->id) selected="selected" @endif>{{$customer->name}}</option>
						@endforeach
					</select>
					@error('customer_name')
				...
				<div class="col-md-6 col-lg-6">
					...
					<label class="font-weight-bold d-block">Sale Date</label>
							...
							<input type="date" name="date" id="date" class="form-control @error('date') @enderror" value="{{ $waysale_details->waysale_date }}">
			...
			<div class="row my-2">
				...
				<div class="col-md-4 col-sm-4 col-lg-4">
					...
					<select name="sale_method" id="sale_method" class="form-control @error ('sale_method') @enderror">
						...
						<option value="cash" @if($waysale_details->credit_method == "cash") {{'selected'}} @endif>Cash Down Payment</option>
						<option value="credit" @if($waysale_details->credit_method == "credit") {{'selected'}} @endif>1 Month Credit Payment</option>
						
						<option value="1week" @if($waysale_details->credit_method == "1week") {{'selected'}} @endif>1 Week Credit Payment</option>
						<option value="2week" @if($waysale_details->credit_method == "2week") {{'selected'}} @endif>2 Week Credit Payment</option>
				...
				<div class="col-md-4 col-sm-4 col-lg-4">
					...
					<input type="number" name="bonus" class="form-control" min="0" placeholder="Enter Bonus" value="{{$waysale_details->bonus}}">
				</div>

				<div class="col-md-4 col-sm-4 col-lg-4">
					<label class="font-weight-bold d-block">Discount Amount</label>
						
					<input type="number" name="discount" class="form-control" min="0" max="100" placeholder="Enter Percentage" value="{{$waysale_details->discount}}">
				</div>
							...
							@foreach($waysale_details->waycreditsaledetails as $detail)
								<tr>
									...
									<td>
										...
										@foreach($way_stock->wayoutdetails as $check_stock) 
											@if($detail->product_id == $check_stock->product_id)
												
											
											<input type="number" name="quantity{{$i}}" value="{{$detail->quantity}}" class="form-control" min="0" max="{{$detail->quantity + $check_stock->quantity}}">
		...
		@php $i=1; @endphp
							...
							<tbody>
								@foreach($promotions as $promo)
									<tr>
										...
										<td>
											<input type="text" name="promo_quantity{{$i}}" value="{{$promo->quantity}}" class="form-control {{$promo->product_id}}">
										</td>
			...
			@elseif(count($promotions) == 0 && $promotion != Null)
				<div class="container">
					<div class="col-md-12 col-lg-12 col-sm-12 text-center">
					...
					<div class="col-md-12 col-lg-12 col-sm-12 table-responsive" id="promo_product">
						<table id="dataTable" class="table table-bordered table-hover">
							...
							<tbody>
								@php $i=1; @endphp
								@foreach($way_stock->wayoutdetails as $row)
									<tr>
										<td>
											{{$i}}
										</td>
