...
@section('content')
	<h1 class="text-center font-weight-bold"><u>Edit Credit Sale Form <span class="text-primary">({{$creditsale->b_short}}-{{$creditsale->voucher_no}})</span></u></h1>
	<div class="container">
		<div class="row">
			<div class="col-md-12 col-lg-12 col-sm-12">
				<a href=" {{ route('sale_branch',$creditsale->branch_id) }} " class="btn btn-outline-warning">Back</a>
			</div>
		</div>
	</div>

	<form method="post" action="{{route('creditsales.update',$creditsale->id)}}" class="form-group">
		@csrf
		@method('PUT')
			...
			<div class="row">
				<div class="col-md-6 col-lg-6">
					...
					<select name="customer_name" id="customer_name" class="form-control @error ('customer_name') @enderror">
						...
						@foreach($customers as $customer)
							...
							<option value="{{$customer->id}}" @if($creditsale->customer_id == $customer->id) selected="selected" @endif>{{$customer->name}}</option>
						@endforeach
					</select>
					@error('customer_name')
				...
				<div class="col-md-6 col-lg-6">
					...
					<label class="font-weight-bold d-block">Sale Date</label>
							...
							<input type="date" name="date" id="date" class="form-control @error('date') @enderror" value="{{ $creditsale->saledate }}">
			...
			<div class="row my-2">
				...
				<div class="col-md-4 col-sm-4 col-lg-4">
					...
					<select name="sale_method" id="sale_method" class="form-control @error ('sale_method') @enderror">
						...
						<option value="cash" @if($creditsale->credit_method == "cash") {{'selected'}} @endif>Cash Down Payment</option>
						<option value="credit" @if($creditsale->credit_method == "credit") {{'selected'}} @endif>1 Month Credit Payment</option>
						<option value="1week" @if($creditsale->credit_method == "1week") {{'selected'}} @endif>1 Week Credit Payment</option>
						<option value="2week" @if($creditsale->credit_method == "2week") {{'selected'}} @endif>2 Week Credit Payment</option>
				...
				<div class="col-md-4 col-sm-4 col-lg-4">
					...
					<input type="number" name="bonus" class="form-control" min="0" placeholder="Enter Bonus" value="{{$creditsale->bonus}}">
				</div>

				<div class="col-md-4 col-sm-4 col-lg-4">
					<label class="font-weight-bold d-block">Discount Amount</label>
						
					<input type="number" name="discount" class="form-control" min="0" max="100" placeholder="Enter Percentage" value="{{$creditsale->discount}}">
				</div>
		...
		@endif
			...
			<input type="submit" class="btn btn-success float-right mr-4" value="Save">
		</div>
